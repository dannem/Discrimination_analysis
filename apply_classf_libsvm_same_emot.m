function [ap, d, c, acc]=apply_classf_libsvm_same_emot(V_sel, idpair,emo,c) %dp
%% c recommended 1.
if nargin<4
    c=1;
end
...
if emo==1 %training on neutral
    ...
    categ_l=idpair(1, 1)*2-1; %preparing indices for two happy idenities for test
    categ_l2=idpair(1, 2)*2-1; %preparing indices for two happy idenities for test
elseif emo==2 %training on happy
    ...
    categ_l=idpair(1, 1)*2; %preparing indices for two neutral idenities for test
    categ_l2=idpair(1, 2)*2; %preparing indices for two neutral idenities for test
else
